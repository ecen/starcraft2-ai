...
class MarineAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        #np.array([[(min(getMinerals(obs), 100) - 50) / 50, getFreeWorkers(obs) / 20, getSupplyFree(obs) / 20,
        #           self.justSelectWorker]])

        if self.justSelectWorker == 1:
            self.action = 3
        elif (getFreeWorkers(obs) > 0):
            self.action = 2
        elif getSupplyFree(obs) < 3 and getMinerals(obs) > 100:
            self.action = 4
        else:
            self.action = 1
